...
void uTurnUntilLine() {
  ...
  while (true) {
    byte s = readSensor();
    if (bitRead(s, 2) || bitRead(s, 3) || bitRead(s, 4) || bitRead(s, 5)) break;
  }
}
...
bool isEndMaze(byte s) {
  return (s == 0b11111111 || s == 0b01111110 || s == 0b01111100 ||
          s == 0b00111110 || s == 0b00111100 || s == 0b00111111 ||
          s == 0b11111100);
}
  ...
  do {
    ...
    for (int i = 1; i < pathlength - 1; i++) {
      if (path[i] == 'U') {
        ...
        else if ((before == 'L' && after == 'R') || (before == 'R' && after == 'L')) {
          path[i - 1] = 'U';
        }
        ...
        // Geser array: hapus path[i] dan path[i+1]
